...
const enableBucketService = async (userId: string, storageInGB: number, accesskey: string, secretkey: string) => {
    ...
    return { StorageEndpoint: `https://minio-${userIdSanitized}.${process.env.HOSTING_DOMAIN!}`, serviceName: `minio-service-${userIdSanitized}`, ingressEndpoint: `https://minio-${userIdSanitized}.${process.env.HOSTING_DOMAIN!}` };
}
